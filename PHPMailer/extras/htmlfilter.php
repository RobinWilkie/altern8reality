 ...
 * @return array%7Cboolean  Returns a false if no matches found, or an array
...
function tln_findnxreg($body, $offset, $reg)
{
    ...
    if (!isset($matches[0]) %7C%7C !$matches[0]) {
        $retarr = false;
    } else {
 ...
 * @param integer $offset Start looking from here.
 * @return array%7Cboolean false if no more tags exist in the body, or
...
function tln_getnxtag($body, $offset)
{
    ...
    while ($pos <= strlen($body)) {
         ...
         */
        $matches = array();
        if (preg_match('%^(\s*)(>%7C/>)%s', substr($body, $pos), $matches)) {
            /**
             * Yep. So we did.
             */
        ...
        default:
            ...
            } elseif (preg_match('%7C[\w/>]%7C', $char)) {
                /**
...
function tln_defang(&$attvalue)
{
    ...
    do {
        $m = false;
        $m = $m %7C%7C tln_deent($attvalue, '/\&#0*(\d+);*/s');
        $m = $m %7C%7C tln_deent($attvalue, '/\&#x0*((\d%7C[a-f])+);*/si', true);
        $m = $m %7C%7C tln_deent($attvalue, '/\\\\(\d+)/s', true);
    } while ($m == true);
...
function tln_fixurl($attname, &$attvalue, $trans_image_path, $block_external_images)
{
    ...
    if ($attvalue && ($attvalue[0] =='"'%7C%7C $attvalue[0] == "'")) {
    ...
    if ($attvalue == '') {
        ...
        if (preg_match('/[\0-\37\200-\377]+/',$attvalue)) {
            ...
            if (isset($aUrl['scheme'])) {
                ...
                if (!isset($aUrl['path']) %7C%7C $aUrl['path'] != $trans_image_path) {
                    $$attvalue = $sQuote . $trans_image_path . $sQuote;
                }
            }
        }
...
function tln_fixstyle($body, $pos, $trans_image_path, $block_external_images)
{
     ...
     */
    $content = preg_replace("%7Cbody(\s*\{.*?\})%7Csi", ".bodyclass\\1", $content);

    /**
    * Fix url('blah') declarations.
    */
    //   $content = preg_replace("%7Curl\s*\(\s*([\'\"])\s*\S+script\s*:.*?([\'\"])\s*\)%7Csi",
            ...
            if ($tagtype == 2) {
                ...
                if ($skip_content == false) {
                    ...
                    if ($tagtype == 1
                        ...
                        if (($rm_tags == false
                             && in_array($tagname, $tag_list)) %7C%7C
                            ($rm_tags == true
...
function HTMLFilter($body, $trans_image_path, $block_external_images = false)
{
    ...
    if ($block_external_images) {
        array_push(
            $bad_attvals{'/.*/'}{'/^src%7Cbackground/i'}[0],
            '/^([\'\"])\s*https*:.*([\'\"])/si'
        );
        array_push(
            $bad_attvals{'/.*/'}{'/^src%7Cbackground/i'}[1],
